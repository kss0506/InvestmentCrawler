...
async def test_ticker(ticker):
    ...
    try:
        ...
        if "관련 뉴스:" not in result:
            ...
            result = result.replace(" (PLTR)$", " (PLTR) $")
            result = result.replace(" (ORCL)$", " (ORCL) $")

            # Fix formatting for dates in stock information
            if "2025년 03월 27일 종가2025년" in result:
                result = result.replace("2025년 03월 27일 종가2025년", "2025년 03월 27일 종가\n2025년")
